...
public class QuerqySolrContainer extends SolrContainer {
    ...
    public static Collection<DockerImageName> getSolrTestVersions() {
        return Arrays.asList(
                        System.getProperty(QuerqySolrContainer.PROP_QUERQY_SOLR_TEST_IMAGES)
                                .split(",")).stream()
...
//                    String.format("/opt/solr/server/solr/configsets/%s", QUERQY_IT_CONFIGSET));
    ...
    public String getSolrUrl() {
        String format = String.format("http://%s:%s/solr", getContainerIpAddress(), getSolrPort());
        return format;
    }
    ...
    public Path getTestDataPath() {
        return Paths.get(String.format("%s/test-classes/integration-test-data/icecat-products-w_price-19k-20201127.json",
