 ...
 /* @SqlMergeMode(MERGE)
  @Sql(scripts = {"/sql/items-books.sql"},
    ...
    void testLoadScripts() {
        List<Item> gItem = itemRepository.findByName("Grocery-1");
        List<Item> bItem = itemRepository.findByName("Book-1");

        Assertions.assertNotNull(gItem.get(0));
        Assertions.assertNotNull(bItem.get(0));
    }
}